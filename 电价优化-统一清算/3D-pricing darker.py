...
# 使用viridis颜色映射，它更柔和且层次丰富
# 或者使用'gnuplot2', 'terrain', 'gist_earth'等更深的颜色
surf = ax.plot_trisurf(R_array, C_array, Pi_array, 
                      triangles=tri.triangles, 
                      cmap='terrain', alpha=0.85,  # 降低透明度，颜色更深
                      edgecolor='none', antialiased=True,
                      vmin=np.min(Pi_array), vmax=np.max(Pi_array),
                      linewidth=0.1)  # 添加轻微的边缘
...
zero_surf = ax.plot_surface(R_zero_masked, C_zero_masked, zero_plane_masked, 
                           color='darkred', alpha=0.3, label='π=0 plane',
                           linewidth=0.5)
...
# 绘制填充等高线 - 使用更深的颜色映射
levels = 100
# 使用更深的颜色映射：'gist_earth', 'terrain', 'ocean', 'gnuplot2'
contourf = ax2.contourf(R_grid, C_grid, Pi_grid_masked, levels=levels, cmap='gist_earth', alpha=0.95)

# 绘制等高线 - 减少数量并淡化
contour_lines = ax2.contour(R_grid, C_grid, Pi_grid_masked, levels=8, colors='white', alpha=0.3, linewidths=0.5)

# 特别标出pi=0的等高线 - 使用深色
zero_contour = ax2.contour(R_grid, C_grid, Pi_grid_masked, levels=[0], colors='darkred', linewidths=3, linestyles='-')
ax2.clabel(zero_contour, inline=True, fontsize=12, fmt='π=0', colors='darkred')
...
ax2.plot(r_boundary, c_boundary, 'navy', linewidth=2, label='有效区域边界', alpha=0.8)
ax2.plot(r_boundary, c_mid_boundary, 'darkgreen', linewidth=2, label='成本计算分界线', alpha=0.8)
...
# 只使用颜色填充，不显示等高线 - 使用更深的颜色映射
im = ax3.contourf(R_grid, C_grid, Pi_grid_masked, levels=levels, cmap='terrain', alpha=1.0)

# 只显示π=0的线 - 使用深色
zero_contour3 = ax3.contour(R_grid, C_grid, Pi_grid_masked, levels=[0], colors='darkred', linewidths=3)
ax3.clabel(zero_contour3, inline=True, fontsize=14, fmt='π=0', colors='darkred')

# 添加边界 - 使用深色
ax3.plot(r_boundary, c_boundary, 'navy', linewidth=2, alpha=0.8)
ax3.plot(r_boundary, c_mid_boundary, 'darkgreen', linewidth=2, alpha=0.8)

ax3.set_xlabel('r', fontsize=14)
ax3.set_ylabel('c', fontsize=14)
ax3.set_title('利润分布图（颜色越深表示利润越高）', fontsize=16)
