...
class ElectricityMarket_W:
    ...
    def calculate_profit(self, P_W_bid, bids_W):
        ...
        for t in range(self.n_periods):
            ...
            revenue = (p_bid + p_setting) / 2 * alloc_W
            cost = 200 * alloc_W + max(0, 300 * (P_W_bid_t - P_W_t))
    ...
    def optimize_strategy_with_smoothing(self, bids_W_0, lambda_smooth=0.15):
        ...
        def objective(x):
            ...
            for i in range(len(bids_W) - 1):
                ...
                if price_change > 20:  # 阈值
                    smoothing_penalty += (price_change - 20) ** 2
        ...
        for i in range(self.n_periods):
            if i == 0:
                ...
                # 基于前一时段报价设置边界
                prev_bid = bids_W_0[i - 1] if bids_W_0 is not None else 50
                lower = max(0, prev_bid - 30)
                upper = min(300, prev_bid + 30)
                bounds.append((lower, upper))

        # 初始猜测
        x0 = np.concatenate([
            self.P_W_pre,
            bids_W_0 if bids_W_0 is not None else np.full(self.n_periods, 50)
        ])
