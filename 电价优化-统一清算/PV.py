...
class ElectricityMarket:
    def __init__(self, demand, P_W_pre, P_PV_pre, bids_W, delta_PV_0):
    ...
    def calculate_profit(self, P_PV_bid, bids_W, bids_PV):
        """计算风电和光伏的总利润"""
        total_profit = 0
        ...
        for t in range(self.n_periods):
            ...
            for bid, qty, stype in suppliers_sorted:
                ...
                # 直接计算三个区间的分配量
                    ...
                    cost = 8 * alloc + max(0, 300 * (P_PV_bid_t - P_PV_t))
    ...
    def optimize_strategy(self, bids_PV_0):
        ...
        n_vars = 2 * self.n_periods  # 3个时段的P_bid_PV + 3个时段的bid_PV
        ...
        def objective(x):
            ...
            # 计算总利润（负号因为我们要最小化负利润）
            profit = self.calculate_profit(P_PV_bid, bids_W, bids_PV)
            return -profit
        ...
        for i in range(n_periods):
            bounds.append((self.P_PV_act[i] - 3, self.P_PV_act[i] + 3))
...
if __name__ == "__main__":
    ...
    P_W_pre = ration * P_W_portion / total_W_portioin * total_demand # 风电预测出力

    P_PV_portion = df['PV']
    total_PV_portioin = np.sum(P_PV_portion)
    P_PV_pre = ration * P_PV_portion / total_PV_portioin * total_demand # 光伏预测出力

    bids_W = W_price_predict(df)
    
    # 创建市场实例
    market = ElectricityMarket(demand, P_W_pre, P_PV_pre, bids_W, delta_PV_0)
    
    print("=== 市场参数 ===")
    print(f"需求: {demand}")
    print(f"风电预测: {P_W_pre}")
    print(f"光伏预测: {P_PV_pre}")
    # print(f"光伏投标范围: [{market.P_PV_act_min}, {market.P_PV_act_max}]")
    bids_PV_0 = df['PV_Ave']
    # 优化策略
    print("\n=== 开始优化 ===")
    ...
    if result.success:
        ...
        # 解析结果
        P_PV_act_opt = result.x[0:n_periods]
        bids_PV_opt = result.x[n_periods:2 * n_periods]
        total_profit = -result.fun
        
        print(f"\n=== 最优策略 ===")
        print(f"风电实际出力: {[f'{p:.2f}' for p in P_PV_act_opt]}")
        print(f"风电报价: {[f'{b:.2f}' for b in bids_PV_opt]}")
        print(f"总利润: {total_profit:.2f}")
        
        # 验证约束
        print(f"\n=== 约束验证 ===")
        for t in range(3):
            within_bounds = (market.P_PV_act_min[t] <= P_PV_act_opt[t] <= market.P_PV_act_max[t])
            print(f"时段{t}: P_PV_act={P_PV_act_opt[t]:.2f}, 约束范围[{market.P_PV_act_min[t]}, {market.P_PV_act_max[t]}], 满足约束: {within_bounds}")
...
def scenario_analysis():
    ...
    for scenario_name, scenario_demand in scenarios.items():
        print(f"\n--- {scenario_name} ---")
        market_scenario = ElectricityMarket(scenario_demand, P_W_pre, P_PV_pre, bids_W)
        ...
        if result.success:
            ...
            print(f"光电出力: {[f'{p:.2f}' for p in P_PV_act_opt]}")
            print(f"光电报价: {[f'{b:.2f}' for b in bids_PV_opt]}")
            print(f"总利润: {total_profit:.2f}")
