...
def Master(n, time_slots, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, P_W, P_PV, P_delta, P_CDP, P_ES_c, P_ES_d, bids_W,
    ...
    if m.status == GRB.OPTIMAL:
        ...
        # 返回优化后的变量字典（包含 .X 属性）
        return PGC_profit, P_PGC_values, p_max_values[
            0], lambda_VPPO_CDP_values, lambda_VPPO_CDP_up_values, lambda_VPPO_CDP_down_values  # , lambda_VPPO_ES_d_values, lambda_VPPO_ES_c_values
...
# 添加标题、标签和图例
plt.title('Power Allocation Stacked Bar Chart (96 Quarters) under a penetration rate of 40%', fontsize=14)
