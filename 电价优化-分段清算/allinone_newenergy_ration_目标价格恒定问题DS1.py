...
def WPV_sort(demand, R, p_max):
    ...
    # 使PV报价随p_max变化
    bids_PV_act = [bid + random.uniform(-30, 30) for bid in bids_PV_act]
    # 确保报价非负
        ...
        for bid, capacity, supplier_type, period in suppliers_sorted:
            ...
            if Q_cum < D_t / 2:
                ...
                if 4 * D_t / 5 <= Q_cum + alloc <= D_t:
                    revenue = (bid + p_max) / 2 * (D_t / 2 - Q_cum) + (bid + 4 * p_max / 5) / 2 * 3 * D_t / 10 + (
                                bid + p_max / 2) / 2 * (Q_cum + alloc - 4 * D_t / 5)
...
def Master(n, time_slots, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, P_W, P_PV, P_delta, P_CDP, P_ES_c, P_ES_d, bids_W,
           bids_PV, p_setting_W, p_setting_PV, delta_W, delta_PV, nonneg_index, P_deltaS, record_profit=True):
    m = gp.Model("Master")
    ...
    C_VPPO_delta = quicksum(
        (-(1 - nonneg_index[i]) * lambda_VPPO_CDP_up_m[i] * P_deltaS[i]
         + nonneg_index[i] * lambda_VPPO_CDP_down_m[i] * P_deltaS[i])
        for i in range(n)
    )

    # VPPO利用CDP平衡最差情况误差的成本
    worst_cost = gp.quicksum(p_max_m * (delta_W[i] + delta_PV[i]) for i in range(n)) + C_VPPO_delta
    ...
    if m.status == GRB.OPTIMAL:
        ...
        # 返回优化后的变量字典（包含 .X 属性）
        return PGC_profit, P_PGC_values, p_max_m.X, lambda_VPPO_CDP_values, lambda_VPPO_CDP_up_values, lambda_VPPO_CDP_down_values
...
for R in range(80, 81):
    ...
    for iters in range(iterations):
        ...
        PGC_profit, P_PGC, p_max, lambda_VPPO_CDP, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down = Master(
            ...
            P_W, P_PV, P_delta, P_CDP, P_ES_c, P_ES_d, bids_W, bids_PV, p_setting_W, p_setting_PV,
            delta_W, delta_PV, nonneg_index, P_deltaS, record_profit=True
        )
        # 更新其他子问题
        P_W, P_PV, bids_W, bids_PV, remain, delta_PV_0, delta_W_0, allocation_result, p_setting_W, p_setting_PV = WPV_sort(
            P_demand, R, p_max)
