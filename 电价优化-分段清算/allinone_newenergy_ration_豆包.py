# allinone_newenergy_ration_目标价格恒定问题ZPQY_修改版.py
# 主要修改：对称调整策略、平滑出清逻辑
...
from PV_96_豆包 import *  # 导入修改版
from W_96_豆包 import *    # 导入修改版
import random
...
# === 修改1：平滑分段出清函数 ===
def smooth_revenue(Q_cum, alloc, bid, p_max, D_t):
    """
    使用sigmoid函数平滑分段点，避免硬切换导致的波动
    ...
    Args:
        ...
        D_t: 需求量
    """
    sigmoid = lambda x: 1 / (1 + np.exp(-10 * (x - 0.5)))  # 在0.5处快速切换

    # 计算三个价格水平的权重
    w1_start = sigmoid(Q_cum / D_t)
    w1_end = sigmoid((Q_cum + alloc) / D_t)
    w1 = (w1_start + w1_end) / 2  # 分段1的权重

    w2_start = sigmoid((Q_cum + alloc) / D_t * 0.8 - 0.4)  # 在0.5-0.8段
    w2_end = sigmoid((Q_cum + alloc * 2) / D_t * 0.8 - 0.4)
    w2 = max(0, w2 - w1)  # 分段2的权重
    ...
    # 加权平均
    revenue = (w1 * p1 + w2 * p2 + w3 * p3) * alloc
    return revenue
...
def WPV_sort(demand, R, p_max):
    ...
    # === 修改2：对称调整策略（移除强制调整）===
    # 原代码：
    # bids_PV_act = [bid + (p_max - 300) * 0.2 for bid in bids_PV_act]
    # bids_PV_act = [max(0, bid) for bid in bids_PV_act]

    # 修改为：不强制调整，让优化器自主决定
    # bids_PV_act 保持优化结果不变
    pass
    ...
    for t in range(n_periods):
        ...
        for bid, capacity, supplier_type, period in suppliers_sorted:
            ...
            # === 修改3：使用平滑出清函数替代硬编码分段 ===
            # 原代码：使用多个if-else的硬切换逻辑
            # 修改为：使用smooth_revenue函数

            revenue = smooth_revenue(Q_cum, alloc, bid, p_max, D_t)

            alloc_setting = alloc
            if alloc == 0:
                alloc_setting = 1

            p_setting = revenue / alloc_setting
            Q_cum += alloc
...
def Master(n, time_slots, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, P_W, P_PV, P_delta, P_CDP, P_ES_c, P_ES_d, bids_W,
    ...
    if m.status == GRB.OPTIMAL:
        ...
        # 返回优化后的变量字典（包含 .X 属性）
        return PGC_profit, P_PGC_values, p_max_m.X, lambda_VPPO_CDP_values, lambda_VPPO_CDP_up_values, lambda_VPPO_CDP_down_values  # , lambda_VPPO_ES_d_values, lambda_VPPO_ES_c_values
...
plt.bar(x, P_W_pre - P_W, width=width, label='W', color="#9B9B9B", edgecolor='black', bottom=P_W)
...
plt.bar(x, P_PV_pre - P_PV, width=width, label='W', color="#9B9B9B", edgecolor='black', bottom=P_PV)
...
# 涨跌幅度
PGC_Percent = VPPO_profit[iterations] / VPPO_profit[0]
CDP_ES_Percent = CDP_ES_profit_all[iterations + 1] / CDP_ES_profit_all[0]
W_Percent = W_Profit_all[iterations + 1] / W_Profit_all[0]
PV_Percent = PV_Profit_all[iterations + 1] / PV_Profit_all[0]

data = {
    '指标名称': ['PGC_Percent', 'CDP_ES_Percent', 'W_Percent', 'PV_Percent'],
    '百分比值': [PGC_Percent, CDP_ES_Percent, W_Percent, PV_Percent],
    '原始值': [VPPO_profit[iterations], CDP_ES_profit_all[iterations + 1],
               W_Profit_all[iterations + 1], PV_Profit_all[iterations + 1]],
