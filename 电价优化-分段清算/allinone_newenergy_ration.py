        ...
        for bid, capacity, supplier_type, period in suppliers_sorted:
            ...
            if Q_cum < D_t / 2:
                ...
                if D_t / 2 <= Q_cum + alloc <= 4 * D_t / 5:
                    revenue = (bid + p_max) / 2 * (D_t / 2 - Q_cum) + (bid + 4 * p_max / 5) / 2 * (Q_cum + alloc - D_t / 2)
                if 4 * D_t / 5 <= Q_cum + alloc <= D_t:
                    revenue = (bid + p_max) / 2 * (D_t / 2 - Q_cum) + (bid + 4 * p_max / 5) / 2 * 3 * D_t / 10 + (bid + p_max / 2) / 2 * (Q_cum + alloc - 4 * D_t / 5)
            if D_t / 2 <= Q_cum  <= 4 * D_t / 5:
                if Q_cum + alloc <= 4 * D_t / 5:
                    revenue = (bid + 4 * p_max / 5) / 2 * alloc  
                if 4 * D_t / 5 <= Q_cum + alloc <= D_t:
                    revenue = (bid + 4 * p_max / 5) / 2 * (4 * D_t / 5 - Q_cum) + (bid + p_max / 2) / 2 * (Q_cum + alloc - 4 * D_t / 5)
            if Q_cum > 4 * D_t / 5:
...
def Master(n, time_slots, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, P_W, P_PV, P_delta, P_CDP, P_ES_c, P_ES_d, bids_W, bids_PV, p_setting_W, p_setting_PV, record_profit=True):
    m = gp.Model("Master")
    ...
    # 设定优化目标--------------------------------
    m.setObjective(C_PGC_income - C_VPPO_W_cost - C_VPPO_PV_cost - C_VPPO_CDP_cost - C_VPPO_ES_cost - worst_cost, GRB.MAXIMIZE)# 
...
plt.plot(x, P_W+P_PV+P_ES_d+P_TP_base+P_CDP-P_ES_c, 'ro-', linewidth=2, markersize=8, label='Supply Demand (Line)', color="#F8FF38")
