...
class ElectricityMarket_PV:
    ...
    def optimize_strategy(self, bids_PV_0):
        ...
        def objective(x):
            ...
            # === 新增：与W_96_1.py对称的惩罚项 ===
            # 1. 过高报价惩罚：对超过500*0.95=475的报价进行惩罚
            penalty_high = 0.2 * np.sum(np.maximum(0, bids_PV - 500 * 0.95) ** 2)
            # 2. 相邻时段平滑惩罚：避免相邻时段报价波动过大
            penalty_smooth = 0.08 * np.sum(np.diff(bids_PV) ** 2)
            # 3. 边界附近惩罚：防止报价贴着边界
            margin = 30
            penalty_edge = 0.15 * np.sum(
                np.maximum(0, 600 - bids_PV) < margin
            )
        ...
        bounds = []
        # 计算历史风电平均报价作为参考
        avg_wind_bid = np.mean(self.bids_W)

        # 动态边界:以风电平均报价为中心,上下浮动一定比例
        base_price = max(50, avg_wind_bid * 0.8)  # 避免过低
        price_range_upper = min(500, avg_wind_bid * 1.5)  # 上限不超过500
        ...
        for i in range(self.n_periods):
            ...
            # bids_PV的动态边界 - 修改版
        for i in range(self.n_periods):
            # 原代码：upper_bound = min(600, price_range_upper + noise)
            # 修改为：更窄的边界，降低噪声影响
            noise = np.random.normal(0, 10)  # 引入±10的随机扰动
            # 使用绝对值降低噪声影响，并缩小上限
            upper_bound = min(550, price_range_upper + abs(noise) * 0.5)  # 从600→550
    ...
    def optimize_solar_strategy(demand, P_W_pre, P_PV_pre, delta_PV_0, df, p_max, bids_PV_0=None):
        ...
        if result.success:
            ...
            # === 新增：边界裁剪和平滑处理 ===
            bids_PV_opt = np.clip(bids_PV_opt, 0, 550)

            # 轻微平滑处理，但保留主要特征
            window_size = 3
            if len(bids_PV_opt) > window_size:
                bids_PV_smoothed = np.convolve(bids_PV_opt, np.ones(window_size)/window_size, mode='same')
                # 仅对波动较大的点进行平滑
                volatility = np.abs(np.diff(bids_PV_opt))
                mask_smooth = volatility > np.percentile(volatility, 75)
                bids_PV_opt[mask_smooth] = bids_PV_smoothed[mask_smooth]
