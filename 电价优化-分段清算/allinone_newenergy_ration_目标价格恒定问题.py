...
lambda_G_buy_VPPO = [375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375, 375,
                     ...
                     1350, 1350, 1350, 1350, 1350, 1350, 1350, 1350, 1350, 1238, 1125, 1013, 900, 900, 900, 900, 900,
                     769, 638, 506, 375]
# 设置柔性符合价格约束--------------------------------
# PRM削峰价格限制
...
def Master(n, time_slots, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, P_W, P_PV, P_delta, P_CDP, P_ES_c, P_ES_d, bids_W,
    ...
    # m.addConstr(C_VPPO_PV_cost >= I_G_sell_PV + 1e-6, "VPPO_PV>SELL_PV")
    ...
    if m.status == GRB.OPTIMAL:
        ...
        # 修复：返回优化后的p_max，而不是原始的全局p_max
        new_p_max = p_max_values[0] if p_max_values else p_max
        # 返回优化后的储能价格变量
        return PGC_profit, P_PGC_values, new_p_max, lambda_VPPO_CDP_values, lambda_VPPO_CDP_up_values, lambda_VPPO_CDP_down_values, lambda_VPPO_ES_d_values, lambda_VPPO_ES_c_values
...
for R in range(80, 81):
    ...
    # 修复：删除硬编码覆盖P_deltaS的错误代码，使用真实计算结果
    # 初始计算worst case
    ...
    for iters in range(iterations):
        print(f"\n=== 第 {iters + 1} 次迭代 ===")
        # 修复：接收Master返回的所有更新后的变量，包括储能价格和p_max
        PGC_profit, P_PGC, p_max, lambda_VPPO_CDP, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, lambda_VPPO_ES_d, lambda_VPPO_ES_c = Master(
        ...
        # 修复：每次迭代更新worst case分布，因为lambda参数已经更新
        delta_W, delta_PV, P_deltaS, worst_cost_1, nonneg_index = find_worst_case_distributions(
            Fr_samples, lambda_VPPO_CDP_up, lambda_VPPO_CDP_down, epsilon, p_max)
...
# 修复：绘制迭代过程的利润收敛曲线，正确展示迭代变化
plt.figure(figsize=(10, 6))
plt.plot(range(len(PV_Profit_all)), PV_Profit_all, marker='o', linestyle='-', color='b')
plt.xlabel("Iteration")
plt.ylabel("PV Profit (Objective Value)")
plt.title("Convergence of PV Profit during Iteration")
plt.grid(True)
plt.show()

# 修复：修正时段bid价格图的标签，避免误解
...
plt.title("W Bid Price per Time Slot")
plt.grid(True)
plt.show()
...
plt.xlabel("Time Slot")
plt.ylabel("Bid Price")
plt.title("PV Bid Price per Time Slot")
plt.grid(True)
plt.show()
...
plt.xlabel("Time Slot")
plt.ylabel("Clear Price")
plt.title("W Clear Price per Time Slot")
plt.grid(True)
plt.show()
...
plt.xlabel("Time Slot")
plt.ylabel("Clear Price")
plt.title("PV Clear Price per Time Slot")
plt.grid(True)
plt.show()
