...
class ElectricityMarket_W:
    ...
    def optimize_strategy(self, bids_W_0):
        ...
        def objective(x):
            ...
            # --- 优化新增：惩罚项，避免报价都冲到上限 ---
            # 1. 过高报价惩罚：对超过350的报价进行惩罚，抑制极端高价
            penalty_high = 0.1 * np.sum(np.maximum(0, bids_W - 350) ** 2)
            # 2. 相邻时段平滑惩罚：避免相邻时段报价波动过大
            penalty_smooth = 0.05 * np.sum(np.diff(bids_W) ** 2)

            # 调整利润，惩罚项会降低过高报价的收益
            profit = profit - penalty_high - penalty_smooth
    ...
    def optimize_wind_strategy(demand, P_W_pre, P_PV_pre, delta_W_0, df, p_max, bids_W_0=None):
        ...
        if result.success:
            ...
            # --- 优化新增：加入随机扰动，打破恒定值 ---
            # 针对接近上限的报价，加入微小的正态扰动，模拟市场不确定性
            mask = (bids_W_opt >= 398)  # 识别原本会恒定的区域
            if np.sum(mask) > 0:
                # 均值为0，标准差3的小扰动，不会改变整体价格水平
